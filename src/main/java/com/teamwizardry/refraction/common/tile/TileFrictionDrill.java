...
public class TileFrictionDrill extends TileMod implements ITickable {
	...
	public void handleBeam(Beam beam) {
		...
		int dist = (int) beam.finalLoc.distanceTo(beam.initLoc);
		for (int i = -dist; i < dist; i++) {
			for (int k = -dist; k < dist; k++) {
				for (int j = -1; j > -ConfigValues.BEAM_RANGE; j--) {
					...
					if (world.getBlockState(loc).getBlock() != Blocks.WOOL)
						poses.add(loc);
				}
			}
		}
		...
		world.setBlockState(loc, Blocks.WOOL.getDefaultState());//.destroyBlock(loc, true);
