...
public class Beam
{
	...
	public Beam(World world, EntityLivingBase entity, Color color, boolean hasEffect)
	{
		this.world = world;
		this.initLoc = entity.getPositionVector().addVector(0, entity.getEyeHeight(), 0);
		this.finalLoc = entity.getLook(1).normalize().scale(128).add(initLoc);
		this.color = color;
		this.hasEffect = hasEffect;
		if (world.isRemote)
			return;
		
		RayTraceResult trace = Raycast.cast(entity, entity.getLook(1), 128);
	...
	public Beam(World world, Vec3d initLoc, Vec3d slope, Color color, boolean hasEffect)
	{
		this.world = world;
		...
		{
			this.finalLoc = trace.hitVec;
			if (hasEffect && trace.typeOfHit == RayTraceResult.Type.ENTITY)
			{
	...
	public Beam(World world, double initX, double initY, double initZ, double slopeX, double slopeY, double slopeZ, Color color, boolean hasEffect)
	{
		this(world, new Vec3d(initX, initY, initZ), new Vec3d(slopeX, slopeY, slopeZ), color, hasEffect);
	}

	public Beam(World world, Vec3d initLoc, Vec3d dir, float red, float green, float blue, float alpha, boolean hasEffect)
	{
		this(world, initLoc, dir, new Color(red, green, blue, alpha), hasEffect);
	}

	public Beam(World world, double initX, double initY, double initZ, double slopeX, double slopeY, double slopeZ, float red, float green, float blue, float alpha, boolean hasEffect)
	{
		this(world, initX, initY, initZ, slopeX, slopeY, slopeZ, new Color(red, green, blue, alpha), hasEffect);
	}
