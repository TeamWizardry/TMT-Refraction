...
public class RayTrace {
	...
	public RayTraceResult trace() {
		...
		RayTraceResult blockResult;// world.rayTraceBlocks(origin, lookVec, false, ignoreBlocksWithoutBoundingBoxes, returnLastUncollidableBlock);
			...
			if (!isOrigin(targetPos)) {
				if (!ignoreBlocksWithoutBoundingBoxes || targetState.getMaterial() == Material.PORTAL || targetState.getCollisionBoundingBox(world, targetPos) != Block.NULL_AABB) {
					if (targetBlock.canCollideCheck(targetState, false)) {
						RayTraceResult raytraceresult = targetBlock instanceof ILaserTrace ? ((ILaserTrace) targetBlock).collisionRayTraceLaser(targetState, world, targetPos, start, end) : targetState.collisionRayTrace(world, targetPos, start, end);
